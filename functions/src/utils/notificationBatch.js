 ...
 *
 * Document shape is identical to v1's; only the collection differs (see
 * paths.js for why the v2 queues are separate).
 */
async function addToPendingNotifications(userId, type, data) {
  try {
    // Create a reference to track pending notifications
    const batchRef = admin.firestore().collection(C.pendingNotifications).doc(userId);
 ...
 * Groups all events together to prevent spam when one user confirms/declines
 * multiple events.
 */
