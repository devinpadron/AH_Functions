...
/*
 * Drains the three v2 batch queues.
 *
 * A straight port of the deployed v1 processor against the v2 collections and
 * the v2 token lookup. Every notification title, body and data payload below is
 * byte-identical to v1's, including `screenName: "EventList"` — see the note on
 * the availability branch.
 */
exports.processNotificationBatches = onSchedule({
  ...
  try {
    ...
    const batchSnapshot = await admin.firestore()
      .collection(C.pendingNotifications)
    ...
    const availabilityBatchSnapshot = await admin.firestore()
      .collection(C.pendingAvailabilityNotifications)
      .where('lastUpdated', '<', cutoffTime)
      .get();
    ...
    if (!availabilityBatchSnapshot.empty) {
      ...
      const availabilityPromises = availabilityBatchSnapshot.docs.map(async (doc) => {
        ...
        /*
         * `screenName: "EventList"` is preserved from v1 verbatim. It is NOT in
         * the app's route table (Details | TimeEntryDetails | EmployeeList), so
         * tapping one of these pushes logs "Unknown push screenName" and does
         * not navigate — the banner still displays. That is the existing v1
         * behaviour; changing it here would be an app-side decision.
         */
